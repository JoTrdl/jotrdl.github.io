...
'    vec4 Pf = fract(P.xyxy) - vec4(0.0, 0.0, 1.0, 1.0);',
'    vec2 fade_xy = fade(Pf.xy);',
'    float n_xy = mix(P.x, P.y, fade_xy.y);',
'    return 1.0 * n_xy;',
'}',
'',
'',
...
'		}',
'		//vec3 noiseColor = getFragColor(cnoise(vec2(iGlobalTime + refractionVec.x + uv.x, refractionVec.y + uv.y))).rgb;',
'       vec3 noiseColor = texture2D(iChannel0, vec2(iGlobalTime + refractionVec.x + uv.x, refractionVec.y + uv.y)).rgb;',
'		col = mix(noiseColor, reflectColor, reflectColor);',
'	}',
'	else',
'		//col = getFragColor(cnoise(vec2(iGlobalTime + uv.x, uv.y))).rgb;',
'       col = texture2D(iChannel0, vec2(uv.x, uv.y)).rgb;',
